...
function AccreditationWorkspaceLegacy({ entries, courseOptions }: { entries: AccreditationBoardEntry[]; courseOptions: CourseIndexEntry[] }) {
  ...
  const jurisdictions = Array.from(new Set(entries.map(({ record }) => record.jurisdiction).filter(Boolean))).sort();
  ...
  const saveRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = { organization: String(form.get("organization")), jurisdiction: String(form.get("jurisdiction")), status: String(form.get("status")), approvalNumber: String(form.get("approvalNumber")) || null, topicNumber: form.get("topicNumber") ? String(form.get("topicNumber")) : null, creditHours: Number(form.get("creditHours")), effectiveDate: String(form.get("effectiveDate")) || null, expirationDate: String(form.get("expirationDate")) || null, expectedUpdatedAt: current?.record.updatedAt };
...
export function AccreditationWorkspace({ entries, initialPreferences, canRestore, page, total, authorityMode }: { entries: AccreditationBoardEntry[]; initialPreferences: AccreditationTablePreferences; canRestore: boolean; page: number; total: number; authorityMode: "workbook" | "api" }) {
  ...
  const activeEntries = entries.filter((entry) => !entry.record.archivedAt);
  ...
  const assessedCourses = useMemo(() => {
    const byCourse = new Map<string, AccreditationBoardEntry[]>();
    for (const entry of activeEntries) byCourse.set(entry.course.courseId, [...(byCourse.get(entry.course.courseId) ?? []), entry]);
  ...
  const organizations = Array.from(new Set(activeEntries.map(({ record }) => record.organization))).sort();
  const jurisdictions = Array.from(new Set(activeEntries.map(({ record }) => record.jurisdiction).filter(Boolean))).sort();
  ...
  const saveRecord = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const payload = { organization: value("organization", current?.record.organization ?? "")!, jurisdiction: value("jurisdiction", current?.record.jurisdiction ?? "")!, status: value("status", current?.record.status ?? "Approved")!, approvalNumber: value("approvalNumber", current?.record.approvalNumber ?? null) || null, topicNumber: value("topicNumber", current?.record.topicNumber ?? null) || null, creditHours: Number(value("creditHours", String(current?.record.creditHours ?? 0))), effectiveDate: value("effectiveDate", current?.record.effectiveDate ?? null) || null, expirationDate: value("expirationDate", current?.record.expirationDate ?? null) || null, expectedUpdatedAt: current?.record.updatedAt };
    setPending(true); setMessage("");
  ...
  const restoreRecord = async (entry: AccreditationBoardEntry) => { if (!entry.record.updatedAt) return; setPending(true); try { const response = await fetch(`/api/accreditations/${entry.record.id}/restore`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ expectedUpdatedAt: entry.record.updatedAt }) }); const result = await response.json() as { message?: string }; if (!response.ok) throw new Error(result.message); setMessage(result.message ?? "Accreditation restored."); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Record could not be restored."); } finally { setPending(false); } };

  return <WorkspaceFrame eyebrow="Compliance workspace" title="Accreditation" description={`One parent row per course with organization and jurisdiction history underneath. ${authorityMode === "api" ? "LMS source fields are API-locked." : "Workbook changes require LMS confirmation."}`} action={<div className="button-row"><button className="button button-secondary" onClick={() => setShowArchived((value) => !value)}>{showArchived ? "Hide archived" : `Archived (${archivedEntries.length})`}</button><button className="button button-primary" onClick={() => setEditing("new")}><Award size={16} /> Add record</button></div>}>
    <MetricStrip metrics={[["History records", String(activeEntries.length), "Active current and historical rows"], ["At-risk courses", String(assessedCourses.filter((item) => item.isAtRisk).length), "Distinct courses requiring action"], ["Future records", String(assessedCourses.flatMap((item) => item.groups).filter((group) => group.summary.historyRole === "future").length), "Not yet effective"], ["Archived", String(archivedEntries.length), "Recoverable by administrators"]]} />
    {message && <div className="inline-alert" role="status"><ShieldCheck size={17} /><span>{message}</span></div>}
    {editing && <AccreditationRecordEditor record={editing === "new" ? null : editing.record} courseField={editing === "new" ? <AsyncCourseSelect /> : <label>Course<input disabled value={`${editing.course.courseCode} — ${editing.course.courseTitle}`} /><input type="hidden" name="courseId" value={editing.course.courseId} /></label>} pending={pending} apiLocked={authorityMode === "api"} onSubmit={saveRecord} onCancel={() => setEditing(null)} />}
    {showArchived && <section className="panel"><div className="panel-heading"><div><h2>Archived accreditation history</h2><p>Source evidence is retained and records can be restored by administrators.</p></div></div><div className="table-scroll"><table className="data-table"><thead><tr><th>Course</th><th>Issuing body</th><th>Jurisdiction</th><th>Accreditation</th><th>Archived</th><th>Actions</th></tr></thead><tbody>{archivedEntries.map((entry) => <tr key={entry.record.id}><td><Link href={`/courses/${entry.course.courseId}`}>{entry.course.courseTitle}</Link></td><td>{entry.record.organization}</td><td>{entry.record.jurisdiction || "—"}</td><td>{entry.record.approvalNumber ?? "—"}<small>Topic {entry.record.topicNumber ?? "—"}</small></td><td>{entry.record.archivedAt?.slice(0, 10)}</td><td>{canRestore ? <button disabled={pending} onClick={() => void restoreRecord(entry)}>Restore</button> : <small>Administrator restore</small>}</td></tr>)}</tbody></table></div></section>}
...
export function VersionsWorkspace({ entries, initialPreferences, canRestore, page, total }: { entries: VersionBoardEntry[]; initialPreferences: VersionsTablePreferences; canRestore: boolean; page: number; total: number }) {
  ...
  const restoreVersion = async (entry: VersionBoardEntry) => { if (!entry.version.updatedAt) return; setPending(true); setMessage(""); try { const response = await fetch(`/api/course-versions/${entry.version.id}/restore`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ expectedUpdatedAt: entry.version.updatedAt }) }); const result = await response.json() as { message?: string }; if (!response.ok) throw new Error(result.message); setMessage(result.message ?? "Version restored."); router.refresh(); } catch (error) { setMessage(error instanceof Error ? error.message : "Version could not be restored."); } finally { setPending(false); } };

  const editor = editing && <VersionRecordEditor
    version={editing === "new" ? null : editing.version}
    courseField={editing === "new" ? <AsyncCourseSelect /> : <label>Course<input disabled value={`${editing.course.courseCode} — ${editing.course.courseTitle}`} /><input type="hidden" name="courseId" value={editing.course.courseId} /></label>}
    pending={pending}
  ...
  return (
    <WorkspaceFrame eyebrow="Lifecycle workspace" title="Versions" description="Create and maintain authoritative course-version history inside CourseTrack."
      ...
      <section className="panel versions-panel">
        <div className="panel-heading"><div><h2>Version history</h2><p>Newest publication records first</p></div><div className="button-row"><button className="button button-secondary" onClick={() => setShowArchived((value) => !value)}>{showArchived ? "Show active" : "Archived history"}</button><details className="column-chooser"><summary>Columns</summary><div>{versionsOptionalColumns.map((column) => <label key={column}><input type="checkbox" checked={show(column)} onChange={() => void saveColumns(show(column) ? visibleColumns.filter((item) => item !== column) : [...visibleColumns, column])} />{versionsColumnLabels[column]}</label>)}<div className="button-row"><button onClick={() => void saveColumns([...versionsOptionalColumns])}>Show all</button><button onClick={() => void saveColumns([...DEFAULT_VERSIONS_TABLE_PREFERENCES.visibleColumns])}>Reset</button></div></div></details></div></div>
