...
export default async function AccreditationPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const [result, preferences, authorityMode] = await Promise.all([getAccreditationBoardPage(page, 100), getAccreditationTablePreferences(auth.userId), getLmsAuthorityMode()]);
