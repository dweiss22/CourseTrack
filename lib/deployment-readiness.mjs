...
export function compareMigrationHistory(checkedInRows, appliedVersions, options = {}) {
  ...
  // Checked in but not yet applied. Reported separately from `errors` because
  // it is the one discrepancy that resolves itself: the release workflow
  // applies these moments later. Everything else in `errors` means the
  // database and the repository genuinely disagree. `errors` still contains
  // these too, so `current` and every existing caller are unchanged.
  const pendingMigrations = [];
  for (const row of requiredRows) {
    if (!applied.has(row.version)) {
      pendingMigrations.push(row.filename);
  ...
  return { current: errors.length === 0, errors, pendingMigrations };
}
...
export async function runDeploymentReadiness(options = {}) {
  ...
  // `allowPendingMigrations` exists for exactly one caller: the Vercel build
  // preflight. A git push starts that build immediately, while the release
  // workflow applies the migration seconds later, so the build would otherwise
  // fail on a discrepancy that resolves itself -- and every migration would
  // need a manual redeploy.
  //
  // It never applies to production: production migrations are applied by
  // production-preparation before the merge to main, so by the time main
  // builds they are already there. And it only forgives pending migrations --
  // a database holding migrations the repository does not, or a duplicated or
  // out-of-order history, still fails everywhere.
  const pending = new Set(schema.pendingMigrations ?? []);
  const tolerated = options.allowPendingMigrations === true
    && configuration.target !== "production"
    && pending.size > 0;
  const blockingErrors = tolerated
    ? schema.errors.filter((message) => ![...pending].some((file) => message.includes(file)))
    : schema.errors;

  if (blockingErrors.length > 0) {
    throw new DeploymentReadinessError("The target database migration contract is not current.", blockingErrors);
  }
  return {
    target: configuration.target,
    authenticationConfigured: true,
    databaseReachable: true,
    schemaContractCurrent: pending.size === 0,
